...
`npm run posters:check` es el gate determinista del archivo final: rechaza URLs externas, rutas inexistentes, archivos que no son WebP, pósters horizontales, dimensiones superiores a 480x720 y archivos de más de 100 KiB. Informa como advertencia los WebP válidos fuera de la meta de 40–80 KiB. El localizador también rechaza fuentes menores a 720x1000 para evitar ampliar miniaturas y bloquea `cinesargentinos.com.ar` y todos sus subdominios como origen de imagen: sus endpoints de póster agregan una bandera argentina superpuesta y no son arte publicable. Cines Argentinos puede seguir usándose como evidencia de cartelera, nunca como fuente del póster.
