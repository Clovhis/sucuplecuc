...
- Resolve credits in two passes: first freeze the trustworthy billing, then separately assess whether each credited person's public profile can be safely enriched. A movie's publication floor is one verified director and at least two verified principal actors/performers in `mainCast`; a director counts toward the performer floor only when a source also confirms an on-screen performer/participant role. For documentaries, named central participants may count when their role is verified. No three-performer minimum applies. For animation/anime, use original-language voice credits.
...
- Create/update a `people.json` entry only when the exact identity is sufficiently established, the movie credit is verified, at least one traceable reference can be saved, and a visually inspected local portrait can be safely attributed to that person. Fill every other available field that has reliable evidence; leave unsupported optional fields absent. If those profile requirements cannot be met, do not create a placeholder or partial person entry. Keep a verified director/actor credit in the movie data when appropriate, document `credit -> reason -> evidence searched` in the ledger, and let the movie pass when the one-director/two-actor minimum is independently met. Never delete an existing global person record used by another movie.
- Do not omit a verified director, lead/co-lead, central marketing face, acting-award recipient or required performer merely because a public profile is unavailable. If a required movie credit itself cannot be verified, or fewer than two principal actors remain, stop the candidate. Do not add children or incidental minors to `mainCast` merely to fill a quota or create a public profile for them.
- Inspect every portrait selected for a person record. Use an image showing that individual, not a poster, logo, film still, placeholder or ambiguous group photo. A cropped event/production photo is acceptable only when the source identifies the person and the crop/order is unambiguous.
- Run bounded enrichment after freezing the final credit list, then run the people audit. The enrichment process has a 15-second request timeout by default. `--strict` fails on actual processing/network failures; unresolved person profiles, missing nationality or missing optional birth data are reported as skipped/incomplete profile warnings and do not block a movie whose verified credits meet the director-and-two-actors floor. Do not wait indefinitely or write a partial person record:
...
- Missing public nationality or birth data is a reported gap, not a movie-level audit failure; omit the unsupported field and never use an empty-string or generic placeholder. In every person card or profile surface, omit the birth/age line when birth data is absent. A verified movie credit may remain without a `people.json` profile when identity/profile evidence or a safe portrait is insufficient; the person audit reports the omission as a warning. The movie still requires one verified director and at least two verified principal actors/performers. Do not waive an unverified credit or fabricate a profile to reach that floor.
...
- Resolve the complete director and principal-actor billing before freezing the JSON. Then keep at least one verified director and two verified principal actors/performers in the movie credits. Person-profile availability is a separate enrichment outcome: missing profile, portrait, nationality or birth data must be reported, but cannot block a movie that meets the verified-credit floor. Do not create a partial person entry to satisfy the movie audit.
- Run bounded people enrichment and `npm run audit:movie-people -- --movie <slug>` for every candidate. Collect profile omissions and actual processing failures separately. A failed/uncertain movie credit or a candidate below the one-director/two-actor floor blocks the movie; a person who has insufficient public profile data is omitted from `people.json` and reported without blocking an otherwise valid movie.
