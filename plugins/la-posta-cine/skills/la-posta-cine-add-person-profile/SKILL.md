...
- Keep roles vocabulary stable; connect `knownFor` to existing movie slugs; preserve sourced nationality when available, verified dates, local compact portrait, profile hero, awards, and visible `referenceUrls`. Nationality is optional: enter it only when an explicit public source supports it; otherwise leave the field absent. Do not hand-build directory cards, filters, counts, credits, or URLs.
- Treat missing public birth data as an explicit factual gap, never as a reason to invent a date. When birth data is absent, omit the birth and age facts from every public person card/profile surface; never save or render a generic missing-birth placeholder. Missing exact identity, a traceable reference or a verifiable portrait blocks creation of a dedicated profile; missing nationality alone does not. If public evidence is insufficient for an identity-safe profile, do not create a partial profile.
