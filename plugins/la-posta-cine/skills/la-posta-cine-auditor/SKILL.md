...
2. Treat these as hard stops: malformed schema; a required candidate without an integer `cinepostaScore` from 1 to 10 after score research, a score that conflicts with the highest-priority available source, or a custom score label; missing or non-canonical movie nationality; unverified/missing AR platform; invalid poster/trailer; missing current-year release date; unverified movie credits; any retained director or `mainCast` person without a matching full public name (or source-backed exact alias), traceable HTTP(S) identity reference, and decodable local portrait of at least 200x250 px; fewer than one full-name director and two distinct full-name principal actors/performers with portraits. Count the minimum using the name shown in the movie credit and the catalog display name; a mononym or single-name alias never counts as full-name credit. Incorrect broad/subgenre taxonomy; an invalid `Bélica` war-filter tag; unsupported awards; manual Share/reaction/meter fields; bad recommendation slugs; stale catalog; and any forbidden diff are also hard stops. Person birth data and nationality remain optional. Every person included in the movie's people credits needs a verified portrait; exactly one director plus two retained principal actors/performers is sufficient, and the auditor must not impose a hidden third-performer minimum.
...
Convert percentage/0–100 scores with `clamp(1, 10, round(value / 10))`, 0–5 scores with `clamp(1, 10, round(value × 2))`, and 0–10 scores with `clamp(1, 10, round(value))`. Store the resulting integer only in `cinepostaScore`; the canonical text label is derived by the site. Do not average sources, cherry-pick among sources, infer a number from Fresh/Rotten, a review's adjectives, or editorial opinion, or use the rating for another version/region/year. Record `source URL → exact raw score, rating type, vote count and scale → retrieval date → calculation → stored score` in the evidence ledger/report. The audit must be able to reproduce the stored integer from that entry. If the full sequence yields no verifiable numeric rating, keep the field unset and report it as unresolved; never fabricate a value or silently waive a required-score validation error.
   ...
   Before interpreting the bundled output, run `npm run audit:movie-people -- --movie <slug>` for every explicit candidate. It is a hard gate: every name retained in `director` or `mainCast` must map to a `people.json` record whose public name matches the credit or lists its exact source-backed alias, has a valid HTTP(S) identity reference, and points to a decodable local portrait file under `public/people` of at least 200x250 px. The auditor also requires at least one full-name director and two distinct full-name principal actors/performers with portraits; full-name status must hold in both the actual credit and public display name. A missing portrait or identity must be fixed with a verified source, or that person's credit must be removed from this movie; never leave a portraitless credit or create a placeholder. If removal drops the title below the one-director/two-actor floor, stop the load. `nationalityPrimary` is optional: search for it, store it only when a public source explicitly supports it, otherwise leave it absent and warn. Missing or unverified birth data is also a warning and must not be invented; the UI must omit the birth/age line entirely and must not render a generic missing-data fallback. An image URL, filename, automated enrichment result, or existing file is not proof of identity: visually confirm every portrait beside an independent credit/filmography source and save both image and identity URLs.
...
4. If a candidate claims `Cine` without same-run live verification, invoke `la-posta-cine-cartelera-revalidator` before sign-off. For external evidence retain only `field → URL → fact`, with JustWatch AR first and official AR source only when needed.
   ...
   Do not accept arbitrary cast thinning or silent credit replacement. Audit the complete verified billing choice, then retain only director/cast credits with a matching named person record, a traceable reference, and a visually verified local portrait. The movie needs at least one full-name verified director and two distinct full-name principal actors/performers in `mainCast`, all with portraits; full-name status must hold in the movie billing and catalog display name. A director counts as a performer only when sources establish an on-screen acting/participant role. This exact floor is sufficient; no third performer is required. If any credit has no safe portrait, omit it from this movie's credit field and record the exact source search, even if central; never leave a portraitless movie credit. If the floor cannot be maintained after omissions, block the title. Do not delete the person's global record if another movie uses it.
...
Report candidate paths, total-catalog count when applicable, the semantic genre matrix and confidence decisions, failures/fixes, platform evidence matrix, portraits verified versus retained person credits, omitted credits with reasons and searched sources, optional birth/nationality gaps, validations, and explicit confirmation that source copy was not reused and no forbidden paths changed. Do not hide nonblocking birth-date or image-host warnings; confirm that missing birth data produces no person-card/profile placeholder. `validate:content` runs the local portrait gate for newly added movie files and existing movie files whose `director` or `mainCast` credits changed; still run `audit:movie-people` explicitly for every candidate/revalidation before commit. If publication was requested, verify the remote SHA/workflow/live slug and finish on a clean synchronized `main`.
