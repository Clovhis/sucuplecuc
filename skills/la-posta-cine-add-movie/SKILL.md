...
description: Add one or more La Posta Cine movie entries safely from a plain-language request. Use for single loads and explicit batches; enforce the canonical Cine Posta 1–10 score contract, duplicate-first intake, evidence-led AR availability research, identity-safe people enrichment, original copy, and content-only validation.
---
...
- If title/year is ambiguous, ask one question before researching. Otherwise extract feedback, requested platform/premiere intent, and an explicit Cine Posta score from 1 to 10 when the user provides one. If the title is intentionally unranked, omit `cinepostaScore`; never translate a vague adjective, external rating, review aggregate, genre, franchise reputation, or platform placement into a score.
