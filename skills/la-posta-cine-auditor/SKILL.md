...
2. Treat these as hard stops: malformed schema; missing or non-canonical movie nationality; unverified/missing AR platform; invalid poster/trailer; missing current-year release date; bad people provenance for the required or retained credits; fewer than one verified director plus two verified performers; incorrect broad/subgenre taxonomy; an invalid `Bélica` war-filter tag; unsupported awards; manual Share/reaction/meter fields; bad recommendation slugs; stale catalog; and any forbidden diff. Exactly one director plus two retained principal performers is sufficient; the auditor must not impose a hidden three-performer minimum.
   ...
   Before interpreting the bundled output, run `npm run audit:movie-people -- --movie <slug>`. Missing local portrait, nationality, traceable reference or an unresolved identity is an error for every retained credit, and the movie must retain one verified director plus at least two verified performers. This is a floor: a third performer is optional when it cannot pass the bounded gate. An optional, low-billing or very obscure credit may be absent after bounded research failed to establish a safe identity/portrait; report that omission and do not expect the auditor to waive a person that is still present in the JSON. Missing public birth data is a warning only when the gap is real and is not filled with an invented date; the UI must omit the birth/age line entirely for that person and must not render a generic missing-birth fallback.
...
`editorial.tenSecondTake` is a seven-field hard gate for every explicit candidate: `verdict`, `whatToExpect`, `pace`, `intensity`, `practicalContext`, `forFansOf`, and `notForYouIf`. Audit every field for presence, usable length, exact reuse across the catalog, repeated text inside the same movie, generic legacy markers, copied `verdictLabel`, and at least two title/director/cast anchors across the block. Read the block as criticism: reject a technically complete guide when it is interchangeable, merely converts metadata into prose, repeats a generic recommendation, or contradicts the film. Historical files lacking this new block are reported as migration gaps in a broad scan; do not auto-fill them. Use `--require-ten-second-take` only for an explicitly authorized backfill gate.
...
4. If a candidate claims `Cine` without same-run live verification, invoke `la-posta-cine-cartelera-revalidator` before sign-off. For external evidence retain only `field → URL → fact`, with JustWatch AR first and official AR source only when needed.
   ...
   Do not accept arbitrary cast thinning or silent credit replacement. Audit the complete verified billing choice, then allow the final `mainCast` to retain only the safe, publishable set: at least two principal performers, plus any additional credits whose identity and portrait pass. This exact one-director/two-performer floor is sufficient for publication; no three-performer target may block a candidate. If a low-billing/obscure optional credit cannot pass the bounded identity/portrait gate, omit it from the movie and record the exclusion with the evidence searched. Never omit the director, a lead/co-lead, an acting-award recipient or any person needed to preserve the two-performer minimum; those cases remain hard stops.
