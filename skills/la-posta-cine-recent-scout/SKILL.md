...
description: Find one publishable La Posta Cine release from the last seven days and hand it to the movie-load workflow. Use compact live research, duplicate/recency gating, and the existing add, cartelera, and audit skills; never invent a Cine Posta score during scouting.
---
...
4. For the first passing candidate, add one independent source confirming the date or relevance. Then hand off only title, year, release date, AR context, and the two URLs to `la-posta-cine-add-movie` in quiet mode. Do not pass a draft review, synopsis, old verdict category, external rating, or inferred `cinepostaScore`: the loading skill owns the original editorial work and applies the canonical 1–10 contract only when the movie is actually valued.
