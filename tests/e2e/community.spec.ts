...
test('movie discussion is a static route and stays safe while disabled', async ({ page }) => {
	const response = await page.goto('/comunidad/peliculas/akira-1988/', { waitUntil: 'networkidle' });
	...
	await expect(page.getByRole('button', { name: 'Publicar mensaje' })).toHaveCount(0);
});
